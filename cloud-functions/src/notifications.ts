...
// Fonction programmée pour les notifications du week-end
export const sendWeekendNotifications = functions.scheduler.onSchedule({
  schedule: '0 18 * * 5,6', // Tous les vendredis et samedis à 18h
...
export const sendEventNotification = functions.https.onCall<{
  eventType: 'halloween' | 'valentine' | 'newyear';
}>(async (request) => {
  if (!request.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
