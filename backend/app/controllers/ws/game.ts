...
export default class GamesController {
  ...
  public async getGameState(gameId: string, userId: string): Promise<any> {
    try {
      ...
      // Récupérer les votes
      const votes = await Vote.query().whereIn(
        'question_id',
  ...
  /**
   * Afficher les détails d'un jeu (implémentation alternative)
   */
  async showGame({ response, params }: HttpContext) {
    try {
  ...
  /**
   * Soumettre une réponse (implémentation alternative)
   */
  async submitGameAnswer({ request, response, auth, params }: HttpContext) {
    try {
      ...
      // Note: This check is commented out as currentPlayerId doesn't exist on Game model
      // if (game.currentPlayerId !== user.id) {
      //   return response.forbidden({ error: "Ce n'est pas votre tour" })
      // }

      // Utiliser l'API correcte pour créer une réponse
      // Note: This is commented as the Game model might not have 'answers' relation defined correctly
      // await game.related('answers').create({
      //   playerId: user.id,
      //   answer,
      // })

      // Alternative implémentation utilisant Answer.create directement
      await Answer.create({
        questionId: game.currentRound, // Assurez-vous que c'est la bonne référence
        userId: user.id,
        content: answer,
      })
