...
const transformQuestion = (question: string, index: number): Question => ({
  id: (index + 1).toString(),
  text: question,
  theme: 'discussion',
  roundNumber: index + 1
...
// Hook personnalisé pour les questions de The Hidden Village
export function useTheHiddenVillageQuestions() {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        const questionsRef = doc(db, 'gameQuestions', 'the-hidden-village');
