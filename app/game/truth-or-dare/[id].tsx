...
const VoteProgressBar = ({
  ...
  <View style={styles.progressRow}>
    <View style={styles.progressBarContainer}>
      <View
        style={[styles.progressBar, { width: `${(current / total) * 100}%` }]}
      />
    </View>
    <Text style={styles.cardProgress}>
      {current}/{total}
...
const TruthOrDareChoiceButtons = ({
  onSelect,
}: {
  onSelect: (choice: "verite" | "action") => void;
}) => {
...
export default function TruthOrDareGameScreen() {
  ...
  const handleNextRound = async () => {
    ...
    try {
      ...
      Alert.alert(t("game.error"), t("game.truthOrDare.errorNextRound"));
    }
  };
  ...
  useEffect(() => {
    if (game?.phase === "vote") {
      setVoteTimer(15); // ⚠️ FIX: Augmenté de 10s à 15s
  ...
  useEffect(() => {
    ...
    console.log("[DEBUG] Vérification des votes:", {
      phase: game.phase,
      totalVoters,
      votesCount,
      votes,
    ...
    if ((votesCount >= totalVoters && totalVoters > 0) || totalVoters === 0) {
      console.log("[DEBUG] Tous les votes reçus, validation dans 1s...");
      voteValidationInProgressRef.current = true;
      setVoteHandled(true);
      ...
      const validationTimeout = setTimeout(async () => {
        ...
        try {
          ...
          if (totalVoters === 0) points = 1; // Cas solo
          ...
          if (!nextPlayer) {
            console.error("[DEBUG] Impossible de trouver le prochain joueur");
            return;
          }
          ...
          console.log("[DEBUG] Jeu mis à jour avec succès");
        } catch (error) {
          console.error("[DEBUG] Erreur validation votes:", error);
          voteValidationInProgressRef.current = false; // Permettre une nouvelle tentative
        }
      }, 1000); // ⚠️ FIX: 1 seconde de délai pour l'UI
        ...
        <SafeAreaView style={styles.phaseWithButtonsContainer}>
          ...
          <View style={styles.topContentContainer}>
            <Text style={styles.playerText}>
              {player?.name || t("game.player", "Joueur")}
            </Text>
            <Text style={styles.chooseTaskText}>
              {t("game.truthOrDare.chooseTask")}
            </Text>
          </View>
          ...
          <View style={styles.spectatorChoiceContainer}>
            ...
            <View style={styles.avatarCircle}>
              <Text style={styles.avatarText}>
                {(player?.name || "Joueur").charAt(0).toUpperCase()}
              </Text>
            </View>
            <Text style={styles.spectatorTitle}>
              {player?.name || t("game.player", "Le joueur")}{" "}
              {t("game.truthOrDare.isThinking")}
            </Text>
            <AnimatedEllipsis style={styles.ellipsis} />
            <Text style={styles.spectatorSubtitle}>
              {t("game.truthOrDare.willChoose")}{" "}
              <Text style={{ color: "#FF6600", fontWeight: "bold" }}>
                {t("game.truthOrDare.action")}
              </Text>{" "}
              {t("game.truthOrDare.or")}{" "}
              <Text style={{ color: "#00B7FF", fontWeight: "bold" }}>
                {t("game.truthOrDare.truth")}
              </Text>
              ?
            </Text>
          </View>
  ...
  if (game.phase === "question" || game.phase === "action") {
    ...
    if (currentQuestion && currentQuestion.text) {
      if (typeof currentQuestion.text === "string") {
        ...
        "text" in currentQuestion.text &&
        typeof currentQuestion.text.text === "string"
      ) {
          ...
          <View style={styles.voteCardShadow}>
            <View style={[styles.voteCard, { backgroundColor: cardBgColor }]}>
              ...
              <Text style={styles.voteTitle}>
                {t("game.truthOrDare.voteInProgress")}
              </Text>
              <Text style={styles.voteSubtitle}>
                {t("game.truthOrDare.otherPlayersDecide")}{" "}
                <Text style={{ color: highlightColor, fontWeight: "bold" }}>
                  {playerName}
                </Text>{" "}
                {t("game.truthOrDare.playedGame")}
              </Text>
              <AnimatedEllipsis style={styles.ellipsis} />
              <Text style={styles.voteCount}>
                {votesCount} / {totalVoters} {t("game.truthOrDare.votes")}
              </Text>
            </View>
          </View>
        </View>
          ...
          >
            <View style={styles.voteCardShadow}>
              <View style={[styles.voteCard, { backgroundColor: cardBgColor }]}>
                ...
                />
                <Text style={styles.voteTitle}>
                  {t("game.truthOrDare.vote")}
                </Text>
                <Text style={styles.voteSubtitle}>
                  {t("game.truthOrDare.did")}{" "}
                  <Text style={{ color: highlightColor, fontWeight: "bold" }}>
                    {playerName}
                  </Text>{" "}
                  {t("game.truthOrDare.playedGame")}?
    ...
    return (
      <View style={styles.container}>
        ...
        <Text style={styles.questionText}>
          {t("game.truthOrDare.round")} {game.currentRound} / {game.totalRounds}
        </Text>
        <Text style={styles.questionText}>
          {t("game.truthOrDare.roundEnd")} {playerName}
        </Text>
        {/* Affichage des scores */}
        <View style={styles.scoreBoard}>
          <Text style={styles.scoreBoardTitle}>
            {t("game.truthOrDare.scores")}
          </Text>
  ...
  async function handleChoice(choice: "verite" | "action") {
    ...
    try {
      ...
      if (!firebaseQuestionType) {
        console.error(
          `Type de question inconnu pour la langue ${currentLanguageCode}: ${choice}`,
        );
        Alert.alert(
          t("game.error"),
      ...
      const nextQuestion = getRandomQuestion(
        firebaseQuestionType as "verite" | "action",
      ); // Caster le type pour TypeScript
  ...
  async function handleVote(vote: "yes" | "no") {
    console.log(
      "[DEBUG] handleVote appelé avec vote:",
      vote,
      "id:",
      id,
      "user:",
